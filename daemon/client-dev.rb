...
daemon = TweetStream::Client.new(language: %w(ru)) #, locations: [55,37,56,38])

daemon.on_no_data_received do |message|
  puts 'NODATA: ' + message.inspect
end

daemon.on_inited do |message|
  puts 'INIT: ' + message.inspect
end

daemon.on_reconnect do |message|
  puts 'RECON: ' + message.inspect
end

daemon.on_unauthorized do |message|
  puts 'UNAUTH: ' + message.inspect
end
...
#daemon.on_anything do |message|
#  puts 'ANY: ' + message.inspect
#end

track_phrases = %w(iphone)
daemon.locations(-180,-90,180,90) do |status|
#daemon.track(track_phrases) do |status|
  conn ||= PG.connect( user: config_db['username'], dbname: config_db['database'] )
#  puts status.attrs.inspect
  puts "#{status.user.name}: #{status.lang}: #{status.geo ? status.geo.coordinates : 'na'}: #{status.place ? status.place.inspect : 'na'}: #{status.text}"
next
